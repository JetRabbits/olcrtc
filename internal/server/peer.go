...
func (s *Server) getOrCreatePeerControlSession(peerID string) *peerSession {
	...
	conn, err := muxconn.NewPeerControlUnbound(s.ln, s.keys, peerID)
	if err != nil {
		logger.Warnf("control muxconn init failed for peer %s: %v", peerID, err)
		s.sessMu.Unlock()
		return nil
	}
	if conn == nil {
	...
	session, err := tunnelcore.NewSession(
		conn, tunnelcore.ServerRole, runtime.ControlSmuxConfigWithProfile(runtime.MaxPayload(s.ln), s.resourceProfile),
	)
	if err != nil {
...
func (s *Server) getPeerSession(peerID string) *peerSession {
	...
	conn, err := muxconn.NewPeerWithQueue(s.peerLn, s.keys, peerID, peerDataQueue)
	if err != nil {
		s.sessMu.Unlock()
		logger.Warnf("data muxconn init failed for peer %s: %v", peerID, err)
		return nil
	}
